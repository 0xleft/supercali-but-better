...
function category_tree($category_id) {
	global $table_prefix, $indent, $supercategory, $scategory,$link;
	...
	if (!$query) echo "Database Error : ".$q;
		...
		while ($row=mysqli_fetch_row($query)) {
			...
			if (!$supercategory) {
				...
				if (mysqli_num_rows($qu) > 0) $mod = mysql_result($qu,0,0);
			} else {
				$mod = 3;
			}
			if ($mod > 0) {
...
function group_tree($group_id) {
	global $table_prefix, $indent, $supergroup, $sgroup,$link;
...
function select_place($field) {
	global $table_prefix, $indent,$link;
...
if (!$superview) {
	...
	if ($_POST["mode"] == $lang["add_event"]) {
		if(get_magic_quotes_gpc()) {
			...
			$link_id = mysqli_real_escape_string($link,stripslashes($_POST["venue"]));
			$contact_id = mysqli_real_escape_string($link,stripslashes($_POST["contact"]));
			$description = mysqli_real_escape_string($link,stripslashes($_POST["description"]));
			$category_id = mysqli_real_escape_string($link,stripslashes($_POST["category"]));
			$group_id = mysqli_real_escape_string($link,stripslashes($_POST["group"]));
			$propose = mysqli_real_escape_string($link,stripslashes($_POST["propose"]));
        } else {
          	$title = mysqli_real_escape_string($link,$_POST["title"]);
			$link_id = mysqli_real_escape_string($link,$_POST["venue"]);
			$contact_id = mysqli_real_escape_string($link,$_POST["contact"]);
			$description = mysqli_real_escape_string($link,$_POST["description"]);
			$category_id = mysqli_real_escape_string($link,$_POST["category"]);
			$group_id = mysqli_real_escape_string($link,$_POST["group"]);
			$propose = mysqli_real_escape_string($link,$_POST["propose"]);
        }
		...
		if (!$supergroup) {
			if ($group_id) {
				...
				//echo $q."<br>";
				$gmod = mysql_result(mysqli_query($link,$q),0,0);
				if ($gmod < 2) {
