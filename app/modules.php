...
function showModules() {
	global $dir, $files, $lang, $table_prefix,$link;
	if (is_dir($dir)) {
	...
	// check if $installed_files is an array
	if (!is_array($installed_files)) $installed_files = array();

	$uninstalled = array_diff($files, $installed_files);
	if ($uninstalled) {
		echo "<h3>".$lang["module_uninstalled_modules"]."</h3>";
		foreach($uninstalled as $key => $val) {
			unset($mod);
...
if (!$_SESSION["user_id"]) {
	...
	$query = mysqli_query($link,"SELECT add_users from ".$table_prefix."users where user_id = ".$_SESSION["user_id"]." limit 1");
