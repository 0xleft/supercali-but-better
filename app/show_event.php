...
if ((!$id) or (!ctype_digit($id))) {
	...
	if (mysqli_num_rows($query) < 1) {
		...
		$squery = mysqli_query($link,$q);
		if (!$squery) echo "<p class=\"warning\">Database Error : ".$q."</p>\n";
		else {
			while ($srow = mysqli_fetch_row($squery)) {
...
<?php
echo $lang["category"].": \n";
$cate = mysql_result(mysqli_query($link,"select name from ".$table_prefix."categories where category_id = ".$category_id),0,0);
...
if ($venue_id > 1) {
	...
	$lq = mysqli_query($link,$q);

	echo $lang["venue"].": \n";
	$li = mysqli_fetch_row($lq);
	if ($li[0]) {
