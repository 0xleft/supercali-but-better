...
*/
function mysql_result($search, $row, $field) {
	$i=0;
...
$link = mysqli_connect($h, $u, $p, $d) or die ("Could not connect to database, try again later");

//	this area to set superpermission levels
$query = mysqli_query($link,"SELECT view, post, add_categories, add_groups, add_users from ".$table_prefix."users where user_id = ".$_SESSION["user_id"]." limit 1");
...
if ((is_numeric($_REQUEST["o"]))&& ($_REQUEST["o"]!= 0)) {
	...
	$o = mysql_result(mysqli_query($link,"SELECT module_id from ".$table_prefix."modules where active = 1 order by sequence limit 1"),0,0);
}
if (!$o) {
