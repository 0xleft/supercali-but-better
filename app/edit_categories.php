...
function category_tree_edit($category_id) {
	global $table_prefix, $common_get,$lang,$link;
...
function category_tree_find($category_id,$find) {
	global $table_prefix, $indent, $id,$link;
...
function editCategory($id) {
	global $table_prefix, $fck_editor_path, $fck_editor_toolbar, $lang, $ck_editor_path,$link;
	if ($id != "add") {
		$q = "SELECT * from ".$table_prefix."categories where category_id = ".$id." limit 1";
		$query = mysqli_query($link,$q);
		if (!$query) echo "Database Error : ".$q;
		else $cat = mysqli_fetch_array($query);
...
if (!$_SESSION["user_id"]) {
	...
	$query = mysqli_query($link,"SELECT add_categories from ".$table_prefix."users where user_id = ".$_SESSION["user_id"]." limit 1");
