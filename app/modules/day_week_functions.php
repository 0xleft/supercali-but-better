...
function showGrid($date) {
	...
	if ($start_time[$date]) {
		...
		$scale = 1;
			$wait = 1;
		foreach(array_keys($start_time[$date]) as $t) {

			foreach($start_time[$date][$t] as $id => $value) {
		...
		foreach($sh as $k => $v) {
			...
			reset($start_empty);
			foreach(array_keys($start_empty) as $r) {
				reset($start_empty[$r]);
				foreach($start_empty[$r] as $kk => $vv) {
		...
		$notsowide = $wide-1;
		reset($start_fill);
		foreach(array_keys($start_fill) as $r) {
			foreach($start_fill[$r] as $kk => $vv) {
				$v = $v+30;
				$left = $wide * $r;
				// wrap fix from Vepr
