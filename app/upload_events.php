...
function category_table($category_id) {
	global $table_prefix, $indent, $edit,$link;
...
function category_tree($category_id) {
	global $table_prefix, $indent, $supercategory, $scategory,$edit,$link;
	$q = "SELECT category_id, name from ".$table_prefix."categories where sub_of = ".$category_id." order by name";
	$query = mysqli_query($link,$q);
	if (!$query) echo "Database Error : ".$q;
	else {
...
function group_tree($group_id) {
	global $table_prefix, $indent, $supergroup, $sgroup, $edit,$link;
	$q = "SELECT group_id, name from ".$table_prefix."groups where sub_of = ".$group_id." order by name";
	$query = mysqli_query($link,$q);
	if (!$query) echo "Database Error : ".$q;
	else {
...
function addEvents() {
global $o,$c,$m,$a,$y,$w,$id,$table_prefix,$page_title,$calendar_title,$supergroup, $supercategory, $lang,$link;
...
foreach($start as $k => $v) {
	...
	if (!$supercategory) {
		...
		$qu = mysqli_query($link,$q);
		if (mysqli_num_rows($qu) < 2) $verify = $lang["no_permision_post_events"];
	}
	if (!$supergroup) {
