...
function editGroup($id) {
	global $table_prefix, $fck_editor_path, $fck_editor_toolbar, $lang,$link;
	if ($id != "add") {
		$q = "SELECT * from ".$table_prefix."groups where group_id = ".$id." limit 1";
		$query = mysqli_query($link,$q);
		if (!$query) echo "Database Error : ".$q;
...
if (!$_SESSION["user_id"]) {
	...
	$query = mysqli_query($link,"SELECT add_groups from ".$table_prefix."users where user_id = ".$_SESSION["user_id"]." limit 1");
